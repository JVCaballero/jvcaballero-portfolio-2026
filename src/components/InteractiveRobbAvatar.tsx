...
export const InteractiveRobbAvatar: React.FC<InteractiveRobbAvatarProps> = ({
  ...
  useEffect(() => {
    const handleMouseMove = (e: MouseEvent) => {
      ...
      // Limit pupil radius to max 6.5px
  ...
  // Avatar retains its signature light blueprint color palette in both light and dark mode
  const primaryStroke = '#3525cd';
  const earFill = '#ffffff';
  const glassesFill = '#ffffff';
  const neckFill = '#f7f9fb';
  const pupilFill = '#3525cd';
  const pupilGlint = '#ffffff';
  ...
  return (
    ...
    >
      ...
      {showHatchedBackdrop && (
        <div
          className="absolute -right-6 top-1/4 w-[110%] h-[55%] bg-hatched border border-primary/30 -z-10 rounded-sm pointer-events-none transition-transform duration-500 group-hover:scale-105"
        />
      )}
      ...
      >
        <defs>
          {/* Gradients fixed to signature light colorway */}
        ...
        {isBlinking ? (
          ...
          <g transform={`translate(${pupilOffset.x}, ${pupilOffset.y})`}>
            <circle cx="176" cy="198" r="4.5" fill={pupilFill} />
            <circle cx="177.5" cy="196.5" r="1.5" fill={pupilGlint} />
          </g>
        )}
        ...
        {isBlinking || isWinking ? (
          ...
          <g transform={`translate(${pupilOffset.x}, ${pupilOffset.y})`}>
            <circle cx="234" cy="198" r="4.5" fill={pupilFill} />
            <circle cx="235.5" cy="196.5" r="1.5" fill={pupilGlint} />
          </g>
        )}

        {/* --- NOSE WITH ACCENT HIGHLIGHT --- */}
        <g>
          {/* Nose shadow fill */}
          <path
